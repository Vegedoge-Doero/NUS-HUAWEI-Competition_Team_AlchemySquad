...
if load_model:
    load_checkpoint(torch.load(f'./SavedNN/{useNN}_checkpoint{loadpoint}.pth.tar'), model, optimizer)

# Datasets MNIST
loadStocks = True
view_dataset = False
if loadStocks:
    priceList, msList, VWAP, stock_name = pickData(input_file="../dataset/tickdata_20221229.csv")
    ...
    dataset = RandomTensor(size=size, transform=transforms.ToTensor(), train=False)
    train_dataset, test_dataset = torch.utils.data.random_split(dataset, [8000, len(dataset) - 8000])
    train_dataset.train = True
    test_dataset.train = False
    print("Successfully loaded the data!")
...
for epoch in epochs:
    ...
    if epoch % save_step == 0 and save_model:
        ...
        save_checkpoint(checkpoint, filename=f'./SavedNN/{useNN}_checkpoint{epoch}.pth.tar')
    if epoch == abortpoint:
        checkpoint = {'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
        save_checkpoint(checkpoint, filename=f'./SavedNN/{useNN}_checkpoint{epoch}.pth.tar')
...
def check_accuracy(loader, model):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            indicesB = y < 4
            indicesS = y > 5
            indicesN = torch.logical_and(y >= 4, y <= 5)
            num_correct += (predictions[indicesB[0]] <= 3).sum()  # Earn for >=, not necessarily =
            num_correct += (predictions[indicesS[0]] >= 6).sum()
            num_correct += (torch.logical_and(predictions[indicesN[0]] >= 4, predictions[indicesN[0]] <= 5)).sum()
