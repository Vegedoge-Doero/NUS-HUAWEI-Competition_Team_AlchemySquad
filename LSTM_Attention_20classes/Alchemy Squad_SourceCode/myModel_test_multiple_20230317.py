...
load_checkpoint(torch.load(f'./CSV{numCSV}_{useNN}_class{num_classes}_checkpoint{loadpoint}.pth.tar', map_location=torch.device('cpu')), model, optimizer)
...
for filedate in range(1):
    ...
    while True:
        ...
        if current_point >= startpoint:
            ...
            if tm_ms < 10800000:  # before 14:00:00 (13800s = 230min = 3h50min)
                if predictions > 10 and tm_ms - last_sell_ms[idx] > 75000 and cum_vol_sell[idx] < 100:
                    sell_vol = predictions - 9
                ...
                if predictions < 9 and tm_ms - last_buy_ms[idx] > 75000 and cum_vol_buy[idx] < 100:
                    buy_vol = 10 - predictions
    ...
    if draw_decision:
        ...
        for count in range(100):
            ...
            axs[math.floor(count / 5) % 5, count % 5].set_title(f'{earn:.2f} bp')
            if count % 25 == 24:
                plt.show()
                fig, axs = plt.subplots(5, 5)
